...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/verification/send", async (req, res) => {
    try {
      ...
      if (!phoneNumber || phoneNumber.length < 10) {
        return res
          .status(400)
          .json({ success: false, message: "유효한 전화번호를 입력해주세요." });
      }
  ...
  app.post("/api/verification/verify", async (req, res) => {
    try {
      ...
      if (!stored) {
        return res
          .status(400)
          .json({ success: false, message: "인증번호를 먼저 요청해주세요." });
      }
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      if (!user) {
        return res
          .status(401)
          .json({ success: false, message: "등록된 계정이 없습니다." });
      }
  ...
  app.get("/api/users/:id/payment-status", async (req, res) => {
    try {
      ...
      const paymentRequests = await storage.getPaymentRequestsByUserId(
        req.params.id,
      );
      ...
      const startDate = user.kingMembershipStartDate
        ? new Date(user.kingMembershipStartDate)
        : null;
  ...
  app.post("/api/conversations", async (req, res) => {
    try {
      ...
      if (!user1Id || !user2Id) {
        return res
          .status(400)
          .json({ success: false, message: "두 사용자 ID가 필요합니다." });
      }
  ...
  app.get("/api/conversations/:id", async (req, res) => {
    try {
      ...
      if (!conversation) {
        return res
          .status(404)
          .json({ success: false, message: "대화를 찾을 수 없습니다." });
      }
      return res.json({ success: true, conversation });
    } catch (error) {
  ...
  app.post("/api/conversations/:id/messages", async (req, res) => {
    try {
      ...
      if (!senderId || !receiverId || !content) {
        return res
          .status(400)
          .json({ success: false, message: "메시지 정보가 필요합니다." });
      }
  ...
  app.get("/api/payment-requests/user/:userId", async (req, res) => {
    try {
      const paymentRequest = await storage.getPaymentRequestByUser(
        req.params.userId,
      );
      return res.json({ success: true, paymentRequest });
    } catch (error) {
  ...
  app.post("/api/admin/payment-requests/:id/approve", async (req, res) => {
    try {
      ...
      const request = await storage.approvePaymentRequest(
        req.params.id,
        adminId || "admin",
      );
      if (!request) {
        return res
          .status(404)
          .json({ success: false, message: "결제 요청을 찾을 수 없습니다." });
      }
  ...
  app.post("/api/admin/payment-requests/:id/reject", async (req, res) => {
    try {
      ...
      const request = await storage.rejectPaymentRequest(
        req.params.id,
        adminId || "admin",
        notes,
      );
  ...
  function getMembershipStatus(user: any): {
    status: string;
    statusLabel: string;
    daysLeft?: number;
  } {
  ...
  app.get("/api/admin/members", async (req, res) => {
    try {
      ...
      const paymentInfoByUser = new Map<
        string,
        {
      ...
      allPaymentRequests.forEach((paymentReq) => {
        const existing = paymentInfoByUser.get(paymentReq.userId) || {
          paymentCount: 0,
        };
        ...
        if (paymentReq.status === "pending") {
          ...
          if (
            ...
            (paymentReq.processedAt &&
              existing.approvedRequest.processedAt < paymentReq.processedAt)
          ) {
            existing.approvedRequest = {
              requestedAt: paymentReq.requestedAt!,
      ...
      const members = allUsers.map((user) => {
        ...
        // Timeline data
        const depositDate =
          paymentInfo?.pendingRequest?.requestedAt ||
          paymentInfo?.approvedRequest?.requestedAt;
        ...
        return {
          ...
          depositorName:
            paymentInfo?.pendingRequest?.depositorName ||
            paymentInfo?.approvedRequest?.depositorName,
          isRenewal,
  ...
  app.post("/api/reports", async (req, res) => {
    try {
      ...
      try {
        ...
        console.error(
          "[Report] Failed to capture message snapshot:",
          snapshotError,
        );
      ...
      if (reportCount === 3) {
        console.log(
          `[Auto-Moderation] User ${reportedUserId} received warning (3 reports)`,
        );
        ...
        if (reportedUser?.phoneNumber) {
          try {
            ...
            console.log(
              `[Auto-Moderation] Warning SMS sent to ${reportedUser.phoneNumber}`,
            );
          } catch (smsError) {
            console.error(
              "[Auto-Moderation] Failed to send warning SMS:",
              smsError,
            );
          }
        }
        ...
        console.log(
          `[Auto-Moderation] User ${reportedUserId} suspended for 7 days (${reportCount} reports)`,
        );
        ...
        if (reportedUser?.phoneNumber) {
          try {
            ...
            console.log(
              `[Auto-Moderation] Suspension SMS sent to ${reportedUser.phoneNumber}`,
            );
          } catch (smsError) {
            console.error(
              "[Auto-Moderation] Failed to send suspension SMS:",
              smsError,
            );
          }
        }
  ...
  app.get("/api/blocks/check", async (req, res) => {
    try {
      ...
      const isBlockedByMe = await storage.isUserBlocked(
        userId as string,
        otherUserId as string,
      );
      const hasBlockedMe = await storage.isUserBlocked(
        otherUserId as string,
        userId as string,
      );
  ...
  app.put("/api/admin/cs-memos/:id", async (req, res) => {
    try {
      ...
      if (!updated) {
        return res
          .status(404)
          .json({ success: false, message: "CS 메모를 찾을 수 없습니다." });
      }
  ...
  app.get("/api/admin/action-logs", async (req, res) => {
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: "관리자 행동 로그 조회 중 오류가 발생했습니다.",
        });
    }
  });
  ...
  app.get("/api/admin/action-logs/user/:userId", async (req, res) => {
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: "사용자 행동 로그 조회 중 오류가 발생했습니다.",
        });
    }
  });
  ...
  app.post("/api/admin/action-logs", async (req, res) => {
    try {
      const { adminId, action, targetUserId, targetUserName, details } =
        req.body;
